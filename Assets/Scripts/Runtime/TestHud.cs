    ...
    public sealed class TestHud : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            foreach (var w in _runner.Loadout.Weapons)
            {
                ...
                if (w.IsPerThrow)
                {
                    // 특수킷은 탄창이 없다 — 돈이 곧 탄약이다.
                    bool affordable = eco.Money >= w.UseCost;
                    string cost = affordable
                        ? $"${w.UseCost}"
                        : $"<color=#cc2222>${w.UseCost}</color>";
                    sb.AppendLine($"{mark} [{w.Data.slot}] {w.Data.displayName,-6} 특수킷      투척 {cost}");
                }
                else
                {
            ...
            sb.AppendLine();
            sb.AppendLine("<color=#888888>A / D 전진·후진 · 스페이스 점프 (지나온 길로는 돌아갈 수 없다)</color>");
            sb.AppendLine("<color=#888888>좌클릭 사격 · R 장전 · 1~5 무기 전환 (5번은 조준점에 던진다)</color>");
